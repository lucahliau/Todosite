            ...
            <div @click.outside="isOpen = false" 
                ...
                <div class="px-8 pb-8 space-y-6">
                    ...
                    <div class="flex items-center justify-between py-2">
                        <span class="font-bold text-slate-900 dark:text-slate-100">Dark Mode</span>
                        <button @click="toggleTheme()" class="w-12 h-7 rounded-full transition-colors relative" :class="darkMode ? 'bg-slate-700' : 'bg-slate-200'">
                            <div class="w-5 h-5 bg-white rounded-full absolute top-1 transition-transform" :class="darkMode ? 'left-6' : 'left-1'"></div>
                        </button>
                    </div>

                    <div class="h-px bg-slate-100 dark:bg-slate-800"></div>

                    <button @click="openArchive()" class="w-full flex items-center justify-between py-2 group">
                        <div class="flex items-center space-x-3">
                            <div class="w-8 h-8 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-500">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                            </div>
                            <span class="font-bold text-slate-900 dark:text-slate-100">Trash & Archive</span>
                        </div>
                    ...
                    <div class="space-y-3">
                         <button @click="triggerArchiveCompleted()" class="w-full py-3 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold text-xs hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                            Archive All Completed
                        </button>
                        <button @click="triggerArchiveOld()" class="w-full py-3 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold text-xs hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-[24px] p-5 text-center">
                        ...
                        <h3 class="font-bold text-slate-900 dark:text-white text-sm mb-1">Daily Briefing</h3>
                        <p class="text-[10px] text-slate-400 mb-4">7:00 AM Notification</p>

                        <button @click="handleAction()" 
                                :disabled="isActive"
                                class="w-full py-3 rounded-xl font-bold text-xs transition-all active:scale-[0.98]"
                    ...
                    <div class="text-center space-y-1">
                        <p class="text-[10px] text-slate-300 font-bold uppercase tracking-widest">Version 1.1.0</p>
                    </div>
                </div>
            </div>
        </div>
